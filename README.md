...
7. 当前不推荐使用 WebSocket 传输方式的应用直接科学上网（套 CDN 使用无碍）；其相关 Xray 应用推荐使用 HttpUpgrade 传输方式替代，功能相同且效率更高。
8. 若 CDN 流量中转（基于 WebSocket/HttpUpgrade/gRPC over TLS）使用不可信的 CDN 进行中转，V2Ray/Xray 示例推荐使用自身带加密的 VMess 或 Shadowsocks 协议配置；否则推荐使用自身不带加密的 VLESS 或 Trojan 协议配置。
